...
# CyberMind one-click deploy and start script
...
print_banner() {
    ...
    echo ""
    echo "=========================================="
    echo "  CyberMind Deploy & Start Script"
...
main() {
    ...
    if [ "$USE_HTTPS" -eq 1 ]; then
        info "Starting CyberMind server (HTTPS + HTTP/2, self-signed cert)..."
        note "For plain HTTP, use: $0 --http"
    else
        info "Starting CyberMind server (HTTP)..."
    fi
    echo "=========================================="
    echo ""
