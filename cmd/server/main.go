...
func main() {
	...
	if !*httpsBootstrap && !*httpBootstrap {
		v := strings.TrimSpace(os.Getenv("CYBERMIND_HTTPS"))
	...
	if strings.HasPrefix(cp, "-") {
		fmt.Fprintf(os.Stderr, "Invalid -config path %q.\nIf HTTPS is also needed, use: ./cybermind --https -config config.yaml (-config must be followed by a yaml file path).\n", cp)
		os.Exit(2)
	}
