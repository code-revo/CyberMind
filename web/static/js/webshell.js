...
function resolveWebshellAiStreamRequest() {
    ...
    return apiFetch('/api/config').then(function (r) {
        ...
        if (typeof window.csaiChatAgentMode === 'object' && typeof window.csaiChatAgentMode.normalizeStored === 'function') {
            norm = window.csaiChatAgentMode.normalizeStored(localStorage.getItem('cybermind-chat-agent-mode'), cfg);
        } else {
            var mode = localStorage.getItem('cybermind-chat-agent-mode');
...
function wsInitAgentMode() {
    ...
    apiFetch('/api/config').then(function (r) { return r.ok ? r.json() : null; }).then(function (cfg) {
        ...
        // 标准化当前值
        var stored = localStorage.getItem('cybermind-chat-agent-mode');
        var norm;
        if (typeof window.csaiChatAgentMode === 'object' && typeof window.csaiChatAgentMode.normalizeStored === 'function') {
...
function wsSelectAgentMode(mode) {
    try { localStorage.setItem('cybermind-chat-agent-mode', mode); } catch (e) { /* */ }
...
function wsRefreshSelectors() {
    ...
    wsRenderRoleList();
    wsUpdateProjectButtonLabel();
    var stored = localStorage.getItem('cybermind-chat-agent-mode') || 'eino_single';
