...
test('超长人工审批内容在限高区域内滚动且操作按钮始终可见', () => {
    ...
    assert.equal(zh.hitl.requestVisitLongUrl, '允许 CyberMind-网络空间自主决策引擎 访问此地址？');
    assert.equal(en.hitl.requestVisitLongUrl, 'Allow CyberMind-网络空间自主决策引擎 to visit this address?');
});
