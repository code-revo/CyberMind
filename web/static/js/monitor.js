...
const CHAT_TASK_SYNC_CHANNEL_NAME = 'cybermind-chat-task-sync-v1';
...
function startProcessDetailsLatestFollow(assistantMessageId, options) {
    ...
    const followLatest = function () {
        ...
        if (window.CyberMindChatScroll &&
            typeof window.CyberMindChatScroll.scrollIfPinned === 'function') {
            window.CyberMindChatScroll.scrollIfPinned(true);
        }
    };
...
function toggleProcessDetails(progressId, assistantMessageId) {
    ...
    if (timeline && timeline.classList.contains('expanded')) {
        setTimeout(() => {
            if (window.CyberMindChatScroll && typeof window.CyberMindChatScroll.scrollIntoViewIfFollowing === 'function') {
                window.CyberMindChatScroll.scrollIntoViewIfFollowing(detailsContainer, { behavior: 'smooth', block: 'nearest' });
...
function describeHitlApprovalRequest(data) {
    ...
    let question = hitlApprovalTemplate('hitl.requestGeneric', '允许 CyberMind-网络空间自主决策引擎 调用 {{tool}}？', { tool: rawToolName });
    let primary = '';
    let kind = 'generic';
    if (isBrowser) {
        kind = 'browser';
        question = url
            ? (url.length > 160
                ? hitlApprovalTranslate('hitl.requestVisitLongUrl', '允许 CyberMind-网络空间自主决策引擎 访问此地址？')
                : hitlApprovalTemplate('hitl.requestVisitUrl', '允许 CyberMind-网络空间自主决策引擎 访问 {{url}}？', { url: url }))
            : hitlApprovalTranslate('hitl.requestBrowser', '允许 CyberMind-网络空间自主决策引擎 使用浏览器？');
        primary = url;
    } else if (isCommand) {
        kind = 'command';
        question = hitlApprovalTranslate('hitl.requestCommand', '允许 CyberMind-网络空间自主决策引擎 执行这条命令？');
        primary = command;
    } else if (isFile) {
        kind = 'file';
        question = path
            ? (path.length > 160
                ? hitlApprovalTranslate('hitl.requestModifyLongPath', '允许 CyberMind-网络空间自主决策引擎 修改此文件？')
                : hitlApprovalTemplate('hitl.requestFile', '允许 CyberMind-网络空间自主决策引擎 修改 {{path}}？', { path: path }))
            : hitlApprovalTranslate('hitl.requestFiles', '允许 CyberMind-网络空间自主决策引擎 修改文件？');
        primary = path;
    }
    return {
...
async function reconcileConversationAfterTaskReplay(conversationId, keepFollowing) {
    ...
    if (keepFollowing && window.CyberMindChatScroll) {
        if (typeof window.CyberMindChatScroll.settleToBottomIfFollowing === 'function') {
            window.CyberMindChatScroll.settleToBottomIfFollowing(24);
        } else if (typeof window.CyberMindChatScroll.forceScrollToBottom === 'function') {
            window.CyberMindChatScroll.forceScrollToBottom(false);
        }
    }
...
async function attachRunningTaskEventStream(conversationId) {
    ...
    const attachPromise = (async function () {
        try {
            ...
            if (window.CyberMindChatScroll && typeof window.CyberMindChatScroll.onTaskEventStreamBegin === 'function') {
                window.CyberMindChatScroll.onTaskEventStreamBegin(conversationId, asEl.id, progressId);
            }
            ...
            if (
                window.CyberMindChatScroll &&
                typeof window.CyberMindChatScroll.settleToBottomIfFollowing === 'function' &&
                (typeof window.captureScrollPinState !== 'function' || window.captureScrollPinState())
            ) {
                window.CyberMindChatScroll.settleToBottomIfFollowing(12);
            } else if (
                window.CyberMindChatScroll &&
                typeof window.CyberMindChatScroll.forceScrollToBottom === 'function' &&
                (typeof window.captureScrollPinState !== 'function' || window.captureScrollPinState())
            ) {
                window.CyberMindChatScroll.forceScrollToBottom(false);
            }
            ...
            if (!response.ok) {
                ...
                if (window.CyberMindChatScroll && typeof window.CyberMindChatScroll.onTaskEventStreamEnd === 'function') {
                    window.CyberMindChatScroll.onTaskEventStreamEnd();
                }
                await reconcileConversationAfterTaskReplay(conversationId, true);
                return false;
            }
            ...
            if (replaySawDone && typeof window.loadConversation === 'function' && window.currentConversationId === conversationId) {
                ...
                if (
                    ...
                    window.CyberMindChatScroll.settleToBottomIfFollowing(18);
                } else if (
                    keepFollowingFinalRender &&
                    window.CyberMindChatScroll &&
                    typeof window.CyberMindChatScroll.forceScrollToBottom === 'function'
            ...
            if (ownsCurrentAttach && window.CyberMindChatScroll && typeof window.CyberMindChatScroll.onTaskEventStreamEnd === 'function') {
                window.CyberMindChatScroll.onTaskEventStreamEnd();
            }
