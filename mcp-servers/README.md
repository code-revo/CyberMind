...
This directory contains **standalone MCP (Model Context Protocol) servers**. They speak the standard MCP protocol over stdio (or HTTP/SSE when a server supports it), so **any MCP client** can use them—not only CyberMind, but also **Cursor**, **VS Code** (with an MCP extension), **Claude Code**, and other clients that support MCP.
...
```json
{
  "mcpServers": {
    "reverse-shell": {
      ...
      "args": ["/absolute/path/to/CyberMind-main/mcp-servers/reverse_shell/mcp_reverse_shell.py"]
    }
  }
}
```
