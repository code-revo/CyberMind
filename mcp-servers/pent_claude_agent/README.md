...
AI-powered **penetration testing engineer** MCP server. CyberMind can command it to run pentest tasks, analyze vulnerabilities, and perform security diagnostics. The agent runs a Claude-based AI internally and can be configured with its own MCP servers and tools.
...
Path placeholders: `${PROJECT_ROOT}` = CyberMind root, `${SCRIPT_DIR}` = this script's directory.

## Setup in CyberMind

1. **Paths**  
   Example: project root `/path/to/CyberMind-main`  
   Script: `/path/to/CyberMind-main/mcp-servers/pent_claude_agent/mcp_pent_claude_agent.py`
...
{
  "pent-claude-agent": {
    "command": "/path/to/CyberMind-main/venv/bin/python3",
    "args": [
      "/path/to/CyberMind-main/mcp-servers/pent_claude_agent/mcp_pent_claude_agent.py",
      "--config",
      "/path/to/CyberMind-main/mcp-servers/pent_claude_agent/pent_claude_agent_config.yaml"
    ],
...
3. **Typical workflow**
   - CyberMind calls `pent_claude_run_pentest_task("Scan target 192.168.1.1 for open ports")`.
   - pent_claude_agent starts a Claude agent internally, which may use Bash, nmap, etc.
   - Results are returned to CyberMind.
...
The process talks MCP over stdio; CyberMind starts it the same way when using External MCP.
