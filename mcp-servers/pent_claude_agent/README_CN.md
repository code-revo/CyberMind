...
AI 驱动的**渗透测试工程师** MCP 服务。CyberMind 可指挥 pent_claude_agent 执行渗透测试任务、分析漏洞、进行安全诊断。Agent 内部使用 Claude Agent SDK，可独立配置 MCP、工具等，作为独立的渗透测试工程师运行。
...
路径占位符：`${PROJECT_ROOT}` = CyberMind 项目根目录，`${SCRIPT_DIR}` = 本脚本所在目录。

## 在 CyberMind 中接入

1. **路径**  
   例如项目根为 `/path/to/CyberMind-main`，则脚本路径为：  
...
3. **使用流程示例**
   - CyberMind 调用 `pent_claude_run_pentest_task("扫描目标 192.168.1.1 的开放端口")`。
...
进程通过 stdio 与 MCP 客户端通信；CyberMind 以 stdio 方式启动该脚本时行为相同。
