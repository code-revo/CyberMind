...
// EnsureAllUsersAdmin 为所有用户授予 admin 角色(幂等)。
// 需求:每个用户都拥有 admin 级功能权限,但数据仍按 owner 严格隔离。
// 每次启动都会执行,确保线上历史注册的 operator 用户也升级为 admin。
func (db *DB) EnsureAllUsersAdmin() error {
	now := time.Now()
	if _, err := db.Exec(`
		INSERT OR IGNORE INTO rbac_user_roles (user_id, role_id, created_at)
		SELECT id, ?, ? FROM rbac_users
	`, RBACSystemRoleAdmin, now); err != nil {
		return err
	}
	return nil
}
...
func grantSystemRolePermissions(tx *sql.Tx, permissions map[string]string) error {
	...
	for key := range permissions {
		...
		case strings.HasPrefix(key, "rbac:"), strings.HasPrefix(key, "config:"), strings.HasPrefix(key, "terminal:"), strings.HasPrefix(key, "audit:"):
			continue
		case key == "mcp:write" || key == "mcp:external:execute" || key == "agent:local-execute":
			continue
		case key == "roles:write" || key == "roles:delete" ||
			...
			key == "workflow:write" || key == "workflow:delete":
			continue
...
func (db *DB) UserCanAccessResource(userID, scope, resourceType, resourceID string) bool {
	...
	// 对话严格按归属隔离,忽略 scope(管理员也只见自己的对话)。
	// 列表过滤(appendConversationAccessFilter)与单条校验都必须一致。
	if resourceType == "conversation" {
		return db.UserCanAccessConversationStrict(userID, resourceID)
	}
...
// UserCanAccessConversationStrict 严格判断用户能否访问某个对话,忽略 RBAC scope。
// 语义与 appendConversationAccessFilter 的列表过滤保持一致(owner / 显式分配 / 所属项目),
// 因此管理员(scope=all)同样只能访问自己的对话。
func (db *DB) UserCanAccessConversationStrict(userID, conversationID string) bool {
	userID = strings.TrimSpace(userID)
	...
	// 1) 对话所有者
	if db.userOwnsResource(userID, "conversation", conversationID) {
		return true
	}
	// 2) 显式资源分配
	var n int
	if err := db.QueryRow(`SELECT COUNT(*) FROM rbac_resource_assignments WHERE user_id = ? AND resource_type = 'conversation' AND resource_id = ?`, userID, conversationID).Scan(&n); err == nil && n > 0 {
		return true
	}
	// 3) 通过所属项目(项目 owner 或分配到项目)
	var projectID sql.NullString
	if err := db.QueryRow(`SELECT project_id FROM conversations WHERE id = ?`, conversationID).Scan(&projectID); err != nil {
		return false
	}
	if projectID.Valid && strings.TrimSpace(projectID.String) != "" {
		if db.userOwnsResource(userID, "project", strings.TrimSpace(projectID.String)) {
			return true
		}
		if err := db.QueryRow(`SELECT COUNT(*) FROM rbac_resource_assignments WHERE user_id = ? AND resource_type = 'project' AND resource_id = ?`, userID, projectID.String).Scan(&n); err == nil && n > 0 {
			return true
		}
	}
	return false
...
// UserCanAccessMessageStrict 严格判断用户能否访问某条消息,忽略 scope。
func (db *DB) UserCanAccessMessageStrict(userID, messageID string) bool {
	var conversationID string
	err := db.QueryRow(`SELECT conversation_id FROM messages WHERE id = ?`, strings.TrimSpace(messageID)).Scan(&conversationID)
	if err != nil {
		return false
	}
	return db.UserCanAccessConversationStrict(userID, conversationID)
}

// UserCanAccessProcessDetailStrict 严格判断用户能否访问某条过程详情,忽略 scope。
func (db *DB) UserCanAccessProcessDetailStrict(userID, processDetailID string) bool {
	var conversationID string
	err := db.QueryRow(`SELECT conversation_id FROM process_details WHERE id = ?`, strings.TrimSpace(processDetailID)).Scan(&conversationID)
	if err != nil {
		return false
