...
import (
	...
	"cybermind/internal/handler"
	"cybermind/internal/hitl"
	"cybermind/internal/knowledge"
	"cybermind/internal/logger"
	"cybermind/internal/mcp"
	"cybermind/internal/mcp/builtin"
	"cybermind/internal/monitor"
	"cybermind/internal/multiagent"
	"cybermind/internal/security"
	"cybermind/internal/skillpackage"
...
func New(cfg *config.Config, log *logger.Logger, configPath string) (*App, error) {
	...
	// 配置文件路径必须由入口传入（与 flag -config 一致）。勿再用 os.Args[1]，否则 ./cybermind --https 会把 --https 当成路径。
...
func setupRoutes(
	...
	registerRL := security.NewRateLimiter(5, 1*time.Minute)
	{
		authRoutes.POST("/login", security.RateLimitMiddleware(loginRL), authHandler.Login)
		authRoutes.POST("/register", security.RateLimitMiddleware(registerRL), authHandler.Register)
	...
	}))
	{
		...
		// 个人 AI 通道配置（每用户自带 API key，替代全局共享 key）
		protected.GET("/user/ai-config", security.RequirePermission("auth:self"), rbacHandler.GetUserAIConfig)
		protected.PUT("/user/ai-config", security.RequirePermission("auth:self"), rbacHandler.UpdateUserAIConfig)
