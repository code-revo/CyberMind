...
const hitlAuditAgentPromptBase = `你是 CyberMind 人机协同审计 Agent。审查 Agent 即将执行的工具调用是否会对系统造成实质性损害。
...
func PrintMCPConfigJSON(mcp MCPConfig) {
	...
	out := map[string]interface{}{
		"mcpServers": map[string]interface{}{
			"cybermind": serverEntry,
		},
	}
	b, _ := json.MarshalIndent(out, "", "  ")
	fmt.Println("[CyberMind] MCP 配置（可复制到 Cursor / Claude Code 使用）：")
