...
import (
	...
	"cybermind/internal/agentfinalizer"
...
func (h *AgentHandler) EinoSingleAgentLoopStream(c *gin.Context) {
	...
	runCfg, resolvedAIChannelID, err := h.configForAIChannelForUser(c, req.AIChannelID)
	if err != nil {
		taskStatus = "failed"
		h.tasks.UpdateTaskStatus(conversationID, taskStatus)
		sendEvent("error", err.Error(), nil)
...
func (h *AgentHandler) EinoSingleAgentLoop(c *gin.Context) {
	...
	runCfg, _, err := h.configForAIChannelForUser(c, req.AIChannelID)
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
