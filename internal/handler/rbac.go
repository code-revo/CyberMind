...
type userAIConfigRequest struct {
...
// GetUserAIConfig returns the current user's own AI channel config. The API key
// is never returned in full — only a masked suffix and a presence flag.
func (h *RBACHandler) GetUserAIConfig(c *gin.Context) {
	...
	resp := gin.H{"configured": false, "has_api_key": false}
	if uc != nil {
		resp["configured"] = uc.Configured()
		resp["provider"] = uc.Provider
		resp["base_url"] = uc.BaseURL
		resp["model"] = uc.Model
		resp["max_total_tokens"] = uc.MaxTotalTokens
		resp["max_completion_tokens"] = uc.MaxCompletionTokens
		if strings.TrimSpace(uc.APIKey) != "" {
			resp["has_api_key"] = true
			resp["api_key_masked"] = maskAPIKey(uc.APIKey)
		}
	}
	c.JSON(http.StatusOK, resp)
}

// UpdateUserAIConfig saves the current user's own AI channel config. An empty
// api_key keeps the previously saved key so the user can tweak model/base_url
// without retyping the secret.
func (h *RBACHandler) UpdateUserAIConfig(c *gin.Context) {
	session, ok := security.CurrentSession(c)
	if !ok || strings.TrimSpace(session.UserID) == "" {
		c.JSON(http.StatusUnauthorized, gin.H{"error": "未登录"})
		return
	}
	var req userAIConfigRequest
	if err := c.ShouldBindJSON(&req); err != nil {
	...
	if apiKey == "" {
		if existing, err := h.db.GetUserAIConfig(session.UserID); err == nil && existing != nil {
			apiKey = existing.APIKey
		}
	}
	if err := h.db.UpsertUserAIConfig(session.UserID, req.Provider, apiKey, req.BaseURL, req.Model, req.MaxTotalTokens, req.MaxCompletionTokens); err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
	if h.audit != nil {
		h.audit.RecordOK(c, "user", "update_ai_config", "更新个人 AI 配置", "user", session.UserID, nil)
	}
	c.JSON(http.StatusOK, gin.H{"success": true})
}
