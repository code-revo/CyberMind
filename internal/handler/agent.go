...
import (
	...
	"cybermind/internal/multiagent"
	"cybermind/internal/openai"
	"cybermind/internal/reasoning"
...
// HITLNeedsToolApproval 供危险任务门控：与会话侧人机协同及免审批白名单判定一致。
...
// configForAIChannelForUser resolves the effective AI channel for a request,
// overriding the global channel with the current user's own AI config when the
// user has configured one. Non-admin users without their own AI config are
// rejected instead of silently falling back to the platform's shared key.
func (h *AgentHandler) configForAIChannelForUser(c *gin.Context, channelID string) (*config.Config, string, error) {
	runCfg, resolvedID, err := h.configForAIChannel(channelID)
	if err != nil {
		return runCfg, resolvedID, err
	}
	if h.db == nil {
		return runCfg, resolvedID, nil
	}
	session, ok := security.CurrentSession(c)
	if !ok || strings.TrimSpace(session.UserID) == "" {
		return runCfg, resolvedID, nil
	}
	uc, err := h.db.GetUserAIConfig(session.UserID)
	if err == nil && uc != nil && uc.Configured() {
		runCfg.OpenAI = config.OpenAIConfig{
			Provider:            uc.Provider,
			APIKey:              uc.APIKey,
			BaseURL:             uc.BaseURL,
			Model:               uc.Model,
			MaxTotalTokens:      uc.MaxTotalTokens,
			MaxCompletionTokens: uc.MaxCompletionTokens,
		}
		return runCfg, resolvedID, nil
	}
	// 管理员未配置个人 API 时回退到平台共享通道；普通用户直接拒绝，避免白嫖共享额度。
	if isAdminSession(session) {
		return runCfg, resolvedID, nil
	}
	return runCfg, resolvedID, fmt.Errorf("请先在右上角「用户菜单 → 我的 API 配置」填写你自己的 AI API 后再运行智能体")
}

// isAdminSession reports whether the session carries the built-in admin role.
func isAdminSession(session security.Session) bool {
	for _, role := range session.Roles {
		if role == database.RBACSystemRoleAdmin {
			return true
		}
	}
	return false
...
func (h *AgentHandler) agentConversationAllowed(c *gin.Context, conversationID string) bool {
	session, ok := security.CurrentSession(c)
	return ok && h.db != nil && h.db.UserCanAccessConversationStrict(session.UserID, strings.TrimSpace(conversationID))
}
