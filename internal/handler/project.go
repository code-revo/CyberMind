...
import (
	...
	"cybermind/internal/attackchain"
...
func (h *ProjectHandler) ListProjectConversations(c *gin.Context) {
	...
	offset, _ := strconv.Atoi(c.Query("offset"))
	// 对话严格按归属隔离:即使能查看项目,也只能列出自己有权访问的对话。
	list, err := h.db.ListConversationsForAccess(limit, offset, "", "", projectID, session.UserID, session.Scope)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
	...
	total, _ := h.db.CountConversationsForAccess("", projectID, session.UserID, session.Scope)
	c.JSON(http.StatusOK, gin.H{
		"conversations": list,
		"total":         total,
		"limit":         limit,
