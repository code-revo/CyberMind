...
// requireConversationAccess 校验当前会话用户对该对话有访问权(严格归属,忽略 scope)。
// 不通过时返回 403,调用方应立即 return。
func (h *ConversationHandler) requireConversationAccess(c *gin.Context, conversationID string) bool {
	session, ok := security.CurrentSession(c)
	if !ok || !h.db.UserCanAccessConversationStrict(session.UserID, conversationID) {
...
// requireMessageAccess 校验当前会话用户对该消息所属对话有访问权(严格归属)。
func (h *ConversationHandler) requireMessageAccess(c *gin.Context, messageID string) bool {
	session, ok := security.CurrentSession(c)
	if !ok || !h.db.UserCanAccessMessageStrict(session.UserID, messageID) {
		c.JSON(http.StatusForbidden, gin.H{"error": "无权访问该对话"})
		return false
	}
	return true
...
// requireProcessDetailAccess 校验当前会话用户对该过程详情所属对话有访问权(严格归属)。
func (h *ConversationHandler) requireProcessDetailAccess(c *gin.Context, processDetailID string) bool {
	session, ok := security.CurrentSession(c)
	if !ok || !h.db.UserCanAccessProcessDetailStrict(session.UserID, processDetailID) {
		c.JSON(http.StatusForbidden, gin.H{"error": "无权访问该对话"})
		return false
	}
	return true
...
func (h *ConversationHandler) SetConversationProject(c *gin.Context) {
	...
	if !h.requireConversationAccess(c, id) {
		return
	}
...
func (h *ConversationHandler) GetMessageProcessDetails(c *gin.Context) {
	...
	if !h.requireMessageAccess(c, messageID) {
		return
	}
...
func (h *ConversationHandler) GetProcessDetail(c *gin.Context) {
	...
	if !h.requireProcessDetailAccess(c, id) {
		return
	}
...
func (h *ConversationHandler) DeleteConversationTurn(c *gin.Context) {
	...
	if !h.requireConversationAccess(c, conversationID) {
		return
	}
